...
(defn ^:private user-config
  []
  (config-from (io/file (System/getProperty "user.home") ".gracie.edn")))

(defn ^:private config-from-classpath
  []
  (if-let [res (io/resource "gracie.edn")]
...
(defn kitten-pictures-components [system config]
  (assoc system
    :kitten-pictures
    (->
     (make new-image-repository config :subject "as-kitten")
     (using [])
     (co-using []))))
...
(defn new-system-map
  ...
  (apply system-map
         (apply concat
                (-> {}
                    (kitten-pictures-components config)
...
(defn new-co-dependency-map
  ...
  {:bootstrap-cover-website-website {:router :modular-bidi-router-webrouter
                                     :kitten-pictures :kitten-pictures}})
